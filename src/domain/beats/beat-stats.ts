namespace TimeCoreDomain {
  ...
  export interface BeatStatsSummary {
    ...
    today: (BeatDayStat & { date: string }) | null;
  }
  ...
  export function createBeatStats() {
    ...
    function record(dateKey: string, rec: { label?: unknown; combo?: unknown }): BeatDayStat {
    ...
    function summary(dateKey: string): BeatStatsSummary {
      ...
      for (const k of Object.keys(days)) copy[k] = { ...days[k] };   // 深拷贝：调用方改动不得泄漏进内部状态
