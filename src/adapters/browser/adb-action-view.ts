...
interface AdbActionViewNode {
  ...
  querySelector(selector: string): AdbActionViewNode | null;
  querySelectorAll(selector: string): AdbActionViewNode[];
  addEventListener(type: string, listener: (event: AdbActionViewInputEvent) => void): void;
  appendChild(child: AdbActionViewNode): void;
}

interface AdbActionViewInputEvent {
  target: { value?: string; checked?: boolean } | null;
}

interface AdbActionViewDocument {
  getElementById(id: string): AdbActionViewNode | null;
  createElement(tagName: string): AdbActionViewNode;
  querySelectorAll(selector: string): AdbActionViewNode[];
}

export interface AdbActionViewDevice {
  serial: string;
  name?: string;
  state?: string;
}

export interface AdbActionViewModel {
  actions: AdbAction[];
  devices: AdbActionViewDevice[];
}

export type AdbActionViewField = 'name' | 'enabled' | 'lead' | 'offset' | 'script' | 'x' | 'y' | 'count' | 'gap';

export type AdbActionViewEvent =
  | { kind: 'field'; action: AdbAction; field: AdbActionViewField; value: string | boolean }
  | { kind: 'device-toggle'; action: AdbAction; serial: string }
  | { kind: 'remove'; action: AdbAction }
  | { kind: 'fire'; action: AdbAction }
  | { kind: 'screenshot'; action: AdbAction }
  | { kind: 'open-picker'; action: AdbAction };

export interface AdbActionViewHandlers {
  onEvent(event: AdbActionViewEvent): void;
}

export interface AdbActionView {
  render(model: AdbActionViewModel, handlers: AdbActionViewHandlers): void;
  refreshDevices(model: AdbActionViewModel): void;
}

interface AdbActionViewOptions {
  document?: AdbActionViewDocument;
}

function runtimeDocument(): AdbActionViewDocument {
  const scope = globalThis as typeof globalThis & { document?: AdbActionViewDocument };
  if (!scope.document) throw new Error('AdbActionView requires a document');
  return scope.document;
}

function required(node: AdbActionViewNode | null, selector: string): AdbActionViewNode {
  if (!node) throw new Error('AdbActionView missing ' + selector);
  return node;
}

function valueOf(event: AdbActionViewInputEvent): string {
  return event.target && event.target.value !== undefined ? event.target.value : '';
}

function checkedOf(event: AdbActionViewInputEvent): boolean {
...
function crossStyle(action: AdbAction): string {
...
function setNumberValue(node: AdbActionViewNode, value: unknown, fallback = ''): void {
  const number = finite(value);
  node.value = number === null ? fallback : String(number);
}

function chipsHTML(action: AdbAction, devices: readonly AdbActionViewDevice[]): string {
  if (!devices.length) return '<span class="dim small">无设备</span>';
...
function cardBody(action: AdbAction, isAdvanced: boolean): string {
  if (isAdvanced) {
...
export function createAdbActionView(options: AdbActionViewOptions = {}): AdbActionView {
  const doc = options.document || runtimeDocument();
  let model: AdbActionViewModel = { actions: [], devices: [] };
  let handlers: AdbActionViewHandlers = { onEvent: () => {} };

  function emit(event: AdbActionViewEvent, card?: AdbActionViewNode): void {
    handlers.onEvent(event);
    if (event.kind === 'field' && (event.field === 'x' || event.field === 'y') && card) {
      const cross = card.querySelector('.a-cross');
      if (cross) cross.style.cssText = crossStyle(event.action);
    }
    if (event.kind === 'device-toggle') refreshDevices(model);
  }

  function bindDevices(box: AdbActionViewNode, action: AdbAction): void {
    box.querySelectorAll('.a-devchip').forEach(chip => chip.addEventListener('click', () => {
      const serial = chip.dataset.serial || '';
      if (serial) emit({ kind: 'device-toggle', action, serial });
    }));
  }

  function actionCard(action: AdbAction): AdbActionViewNode {
    const card = doc.createElement('div');
    card.className = 'adb-action';
    card.dataset.id = action.id;
    const badge = ({ tap: '连点', wake: '亮屏连点', adv: '脚本' } as Record<string, string>)[action.type] || '动作';
    ...
    if (isAdvanced) {
      ...
      setNumberValue(x, action.x); setNumberValue(y, action.y);
      ...
      setNumberValue(count, action.n || 5); setNumberValue(gap, action.gap || 400);
  ...
  function refreshDevices(next: AdbActionViewModel): void {
    model = next;
    for (const card of doc.querySelectorAll('.adb-action')) {
      ...
      if (count) count.textContent = action.devs.length ? '指定 ' + action.devs.length + ' 台' : '全部启用设备';
    }
  }

  function render(next: AdbActionViewModel, nextHandlers: AdbActionViewHandlers): void {
    model = next;
    handlers = nextHandlers;
    const box = doc.getElementById('adb-actions');
    if (!box) return;
    box.innerHTML = '';
    for (const action of model.actions) box.appendChild(actionCard(action));
  }
