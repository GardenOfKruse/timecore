...
- **窗口四形态**：同一个窗口在 正常 / 小窗 / 仅时间 / 全屏 之间变形（Ctrl+1/2/3、标题栏 ◉、右键菜单）——不是多个窗口
- **仅时间悬浮钟面**：280×96 纯 时:分:秒.毫秒 悬浮件，字号随窗体等比缩放；双击退出、拖动移动、滚轮缩放（160–1180px）；边框秒环每秒绕行一周、边框呼吸微光；倒计时归零时整窗脉动
- **行星真实太阳**：3D 星球的昼夜随真实时间转动——正午照亮面向你的一侧，午夜夜半球靠冷色月光，晨昏线偏暖（跟随当前显示时区）
- **到点音景**：归零瞬间三层合成音——sub 冲击 + 五声琶音左右展开 + 高频钟簇余韵，是整个装置的 signature moment
...
| 空格 / 左键 | 击拍 | B | 自由节拍器 |
| F | 全屏 | C | 小窗模式 |
| Ctrl+1 | 仅时间（钟面悬浮） | Ctrl+2 | 小窗 480×320 |
| Ctrl+3 | 正常 1180×760 | M | 静音 |
| Esc | 关闭浮层 / 退全屏 | 滚轮 | 钟面形态下缩放 |
