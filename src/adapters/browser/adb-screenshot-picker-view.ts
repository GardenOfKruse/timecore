...
interface AdbScreenshotPickerNode {
  hidden: boolean;
  src: string;
  textContent: string;
  ...
  getContext?(kind: string): AdbScreenshotPickerContext | null;
  querySelector(selector: string): AdbScreenshotPickerNode | null;
  addEventListener(type: string, listener: (event: AdbScreenshotPickerEvent) => void, options?: boolean | { capture?: boolean }): void;
}

interface AdbScreenshotPickerContext {
...
interface AdbScreenshotPickerEvent {
  clientX?: number;
  clientY?: number;
  key?: string;
  target?: unknown;
...
interface AdbScreenshotPickerDocument {
  getElementById(id: string): AdbScreenshotPickerNode | null;
}

interface AdbScreenshotPickerWindow {
  addEventListener(type: string, listener: (event: AdbScreenshotPickerEvent) => void, options?: boolean | { capture?: boolean }): void;
}

export interface AdbScreenshotPickerAction {
  id: string;
  name?: string;
  _shot?: unknown;
  shotW?: unknown;
...
export interface AdbScreenshotPickerHandlers {
  onSelect(action: AdbScreenshotPickerAction, x: number, y: number): void;
  onRetake(action: AdbScreenshotPickerAction): void;
  onClose(action: AdbScreenshotPickerAction): void;
}

export interface AdbScreenshotPickerView {
  open(action: AdbScreenshotPickerAction): void;
  close(): void;
  isOpen(): boolean;
}

interface AdbScreenshotPickerOptions {
  document?: AdbScreenshotPickerDocument;
  window?: AdbScreenshotPickerWindow;
  handlers?: AdbScreenshotPickerHandlers;
}

function runtimeDocument(): AdbScreenshotPickerDocument {
  const scope = globalThis as typeof globalThis & { document?: AdbScreenshotPickerDocument };
  if (!scope.document) throw new Error('AdbScreenshotPickerView requires a document');
  return scope.document;
}

function runtimeWindow(): AdbScreenshotPickerWindow {
  const scope = globalThis as typeof globalThis & { window?: AdbScreenshotPickerWindow };
  if (!scope.window) throw new Error('AdbScreenshotPickerView requires a window');
  return scope.window;
}

function required(node: AdbScreenshotPickerNode | null, id: string): AdbScreenshotPickerNode {
  if (!node) throw new Error('AdbScreenshotPickerView missing #' + id);
  return node;
}
...
function xyText(action: AdbScreenshotPickerAction): string {
  const x = finite(action.x), y = finite(action.y);
  return (x === null ? '—' : String(x)) + ' , ' + (y === null ? '—' : String(y));
}

function crossStyle(action: AdbScreenshotPickerAction): string {
  const x = finite(action.x), y = finite(action.y);
...
export function createAdbScreenshotPickerView(options: AdbScreenshotPickerOptions = {}): AdbScreenshotPickerView {
  ...
  let active: AdbScreenshotPickerAction | null = null;
  ...
  function closeFromUser(): void {
    const action = active;
    hide();
    if (action) handlers.onClose(action);
  }

  function open(action: AdbScreenshotPickerAction): void {
    if (typeof action._shot !== 'string' || !action._shot) throw new Error('AdbScreenshotPickerView requires a screenshot');
    active = action;
  ...
  function isOpen(): boolean { return !picker.hidden && !!active; }

  image.addEventListener('dragstart', event => { if (event.preventDefault) event.preventDefault(); });
  win.addEventListener('resize', layoutFrame);
  image.addEventListener('mousemove', event => {
    if (!active || event.clientX === undefined || event.clientY === undefined) return;
  ...
  retakeButton.addEventListener('click', () => { if (active) handlers.onRetake(active); });
  picker.addEventListener('click', event => { if (event.target === picker) closeFromUser(); });
  win.addEventListener('keydown', event => {
    if (event.key === 'Escape' && isOpen()) closeFromUser();
  }, true);
