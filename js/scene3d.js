...
(function () {
  ...
  // 测试覆盖参数（?moonage/?dayofyear）会话常量：init 时解析一次，不再每帧跑正则（v1.25.0 帧循环卫生）
  const URL_FLAGS = (() => {
  ...
  function updateSun(e) {
    ...
    const doy = URL_FLAGS.dayofyear != null ? URL_FLAGS.dayofyear : (() => { const p = localDateParts(tz, e); return TimeCoreDomain.dayOfYear(p.y, p.m, p.d); })();
  ...
  function moonEpoch(e) {
    return URL_FLAGS.moonage != null ? TimeCoreDomain.MOON_REFERENCE_NEW_MOON_MS + URL_FLAGS.moonage * 86400000 : e;
  }
  function buildMoon() {
    moon = new THREE.Mesh(
  ...
  function buildCore() {
    ...
    const tex = SceneTextures.planetTextures();
    ...
    const gt = SceneTextures.glowTexture(128);
  ...
  function buildRings() {
    ...
    tipGlow = new THREE.Sprite(new THREE.SpriteMaterial({ map: SceneTextures.glowTexture(64), color: 0xffffff, transparent: true, opacity: 0.9, blending: THREE.AdditiveBlending, depthWrite: false }));
    ...
    conf.forEach((c, i) => {
      ...
      if (i < 2) {
        ...
        const trail = new THREE.Points(tg, new THREE.PointsMaterial({
          size: 0.055, map: SceneTextures.glowTexture(64), vertexColors: true, transparent: true, opacity: 0.6,
  ...
  function buildParticles() {
    ...
    pMat = new THREE.PointsMaterial({
      size: 0.055, map: SceneTextures.glowTexture(64), vertexColors: true, transparent: true, opacity: 0.8,
      blending: THREE.AdditiveBlending, depthWrite: false, sizeAttenuation: true
    });
  ...
  function init(canvas) {
    ...
    try {
      ...
      scene.environment = pm.fromEquirectangular(SceneTextures.envTexture()).texture;
    } catch (_) { /* 无环境贴图也能跑 */ }
