...
- **每日击拍统计（v1.9.0）**：设置抽屉被动展示今日击拍数/最高连击/加权准确率，悬停看近 7 日；`tc.beatstats.v1` 本地保存 30 天滚动。
...
- **到点音景昼夜四态（v1.8.0）**：零点琶音按节点时刻的昼夜自动换五声音阶——晨 C 大 / 昼 G 大 / 暮 E 小 / 夜 D 小（边界 5/11/17/23），sub 冲击与钟簇余韵不变；音阶按**节点 epoch**（显示时区）选取而非调用时刻，预排跨边界不出错。
