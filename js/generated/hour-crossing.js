...
(function (TimeCoreDomain) {
    ...
    function hourCrossed(prevEpochMs, nowEpochMs) {
        if (!Number.isFinite(prevEpochMs) || !Number.isFinite(nowEpochMs))
            return false;
        if (nowEpochMs <= prevEpochMs)
            return false;
        return Math.floor(nowEpochMs / 3600000) > Math.floor(prevEpochMs / 3600000);
    }
    TimeCoreDomain.hourCrossed = hourCrossed;
