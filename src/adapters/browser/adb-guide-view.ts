...
interface AdbGuideViewDocument {
  getElementById(id: string): AdbGuideViewNode | null;
}

export interface AdbGuideViewModel {
  enabled: boolean;
  adbOk: boolean;
  adbAncient: boolean;
  adbVersion?: string;
...
export type AdbGuideViewEvent =
  | { kind: 'download' }
  | { kind: 'scan' };

export interface AdbGuideViewHandlers {
  onEvent(event: AdbGuideViewEvent): void;
}

export interface AdbGuideView {
  render(model: AdbGuideViewModel, handlers: AdbGuideViewHandlers): void;
}

interface AdbGuideViewOptions {
  document?: AdbGuideViewDocument;
}

function runtimeDocument(): AdbGuideViewDocument {
  const scope = globalThis as typeof globalThis & { document?: AdbGuideViewDocument };
  if (!scope.document) throw new Error('AdbGuideView requires a document');
  return scope.document;
}

function step(number: number, ok: boolean, text: string, button: string): string {
  return '<div class="ag-step' + (ok ? ' ok' : '') + '"><i>' + (ok ? '✓' : number) + '</i><span>' + text + '</span>' + button + '</div>';
}

export function createAdbGuideView(options: AdbGuideViewOptions = {}): AdbGuideView {
  const doc = options.document || runtimeDocument();
  let handlers: AdbGuideViewHandlers = { onEvent: () => {} };

  function renderStatus(model: AdbGuideViewModel): void {
  ...
  function renderGuide(model: AdbGuideViewModel): void {
    ...
    if (download) download.addEventListener('click', () => handlers.onEvent({ kind: 'download' }));
    const scan = doc.getElementById('ag-scan');
    if (scan) scan.addEventListener('click', () => handlers.onEvent({ kind: 'scan' }));

    const dot = doc.getElementById('adb-dot');
    if (dot) dot.style.background = !model.enabled ? '#777' : model.readyCount ? '#4dffa6' : model.adbOk ? '#39d7ff' : '#ffb347';
  }

  function render(model: AdbGuideViewModel, nextHandlers: AdbGuideViewHandlers): void {
